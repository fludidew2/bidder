...
class InvoicePdf < Prawn::Document
  def initialize(invoice)
   ...
   def header
    text "Invoice ##{@invoice.id}", size: 12, style: :bold, align: :left
    stroke_horizontal_rule
    move_down 20
   end

  def company_info
    text "From:", size: 8, style: :bold
  ...
  def remit_to
    text "Issued On: #{@invoice.created_at.strftime("%B %d, %Y at %I:%M %p")}", size: 5
    due_date = @invoice.created_at + 30.days
    text "Due On: #{due_date.strftime("%B %d, %Y at %I:%M %p")}", size: 5
    move_down 20
  end

  def text_content
    text "Request Description:", size: 8, style: :bold
